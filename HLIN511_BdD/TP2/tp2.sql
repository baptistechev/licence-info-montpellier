...
/*9*/
SELECT COLUMN_NAME FROM ALL_TAB_COLS WHERE TABLE_NAME ='ABONNE';
SELECT COUNT(COLUMN_NAME) FROM ALL_TAB_COLS WHERE TABLE_NAME IN (Select table_name from user_tables);
/*12*/
/*13....*/
ALTER TABLE ABONNE ADD CONSTRAINT num_ab PRIMARY KEY (NUM_AB);
ALTER TABLE EXEMPLAIRE ADD CONSTRAINT NUMERO PRIMARY KEY (NUMERO);
ALTER TABLE EMPRUNT MODIFY ( NUM_AB NOT NULL);
ALTER TABLE EMPRUNT MODIFY ( NUM_EX NOT NULL);
ALTER TABLE EMPRUNT ADD CONSTRAINT (num_ab,num_ex) PRIMARY KEY (NUM_AB,NUM_EX);

/*15*/
CREATE VIEW ABONNE_MONTP AS (SELECT NUM_AB,NOM, PRENOM FROM ABONNE WHERE VILLE='MONTPELLIER' );
INSERT INTO ABONNE_MONTP VALUES (567634, 'LEMONIER','ADAM');
/*16*/
select view_name from all_views where view_name='ABONNE_MONTP';
/*17*/
CREATE VIEW ETAT_EXEMPLAIRE AS (SELECT DISTINCT ETAT FROM EXEMPLAIRE);
